...
int main(int argc, char *argv[]) {
  ...
  input_file_stream = ifstream(argv[2], ios::binary | ios::ate);
  assert(input_file_stream.is_open());

  file_size = input_file_stream.tellg();
  if (static_cast<size_t>(file_size) != checksums.size()) {
    println("Failed!");
  }

  vector<byte> standard_checksums;
  standard_checksums.resize(file_size);

  input_file_stream.seekg(0, ios::beg);
  input_file_stream.read(reinterpret_cast<char *>(standard_checksums.data()),
                         file_size);

  if (checksums != standard_checksums) {
    println("Failed!");
  } else {
    println("Success!");
  }
